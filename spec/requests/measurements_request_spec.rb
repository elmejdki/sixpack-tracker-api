...
RSpec.describe 'Measurements', type: :request do
  ...
  let!(:measurements) { create_list(:measurement, 10, user_id: user.id, measure_id: measure.id) }
  ...
  describe 'POST /measurements' do
    ...
    let(:valid_attributes) do
      # send json payload
      { value: 23, user_id: user.id, measure_id: measure.id }.to_json
    end
    ...
    context 'when the request is invalid' do
      let(:invalid_attributes) { { value: nil }.to_json }
      ...
      it 'returns a validation failure message' do
        expect(json['message'])
          .to match(/Validation failed: Measure must exist, Value can't be blank/)
      end
    end
  end
  ...
  describe 'PUT /measurements/:id' do
    let(:valid_attributes) { { value: 30 }.to_json }
